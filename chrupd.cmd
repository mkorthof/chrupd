...
ENDLOCAL & powershell.exe -NoLogo -NoProfile -Command "$input | &{ [ScriptBlock]::Create( ( Get-Content \"%~f0\" ) -join [char]10 ).Invoke( @( &{ $Args } %PS_BAT_ARGS% ) ) }"
GOTO :EOF
#>

<# -------------------------------------------------------------------------- #>
<# 20180319 MK: Simple Chromium Updater (chrupd.cmd)   										    #>
<# -------------------------------------------------------------------------- #>
...
If ($Args -iMatch "[-/]h") {
  Write-Host "Uses RSS feed from `"$chkSite`" to download and install latest"
	...
	Write-Host "USAGE: $scriptCmd -[editor|channel|getFile]|[crTask|rmTask|shTask|noVbs|confirm]" "`r`n"
